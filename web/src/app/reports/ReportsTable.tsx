            ...
            >
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <div className="font-semibold text-slate-900 text-base leading-tight">{senderName}</div>
                  <div className="text-xs text-slate-500">Reported {formatDate(item.report.created_at)}</div>
                </div>
                ...
                {item.violations.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-1">
                    {item.violations.slice(0, 2).map((v, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center rounded-md border px-2.5 py-1 text-xs font-medium bg-orange-50 text-orange-800 border-orange-200"
                      >
                        {v.title}
                      </span>
                    ))}
                    {item.violations.length > 2 && (
                      <span className="inline-flex items-center rounded-md bg-slate-100 text-slate-700 border border-slate-200 px-2.5 py-1 text-xs font-medium">
                  ...
                  {item.verdict ? (
                    <HoverCard openDelay={100}>
                      <HoverCardTrigger asChild>
                        <button
                          className={`inline-flex items-center rounded-md px-3 py-1.5 text-xs font-medium ${verdictDisplay.bgColor} ${verdictDisplay.color} border ${verdictDisplay.borderColor} w-fit cursor-pointer hover:opacity-80 transition-opacity`}
                        >
                          {verdictDisplay.label}
                        </button>
                      </HoverCardTrigger>
                      <HoverCardContent className="w-[90vw] max-w-[320px] bg-white border-slate-200 shadow-xl" align="start" sideOffset={8}>
                    ...
                    <HoverCard openDelay={100}>
                      <HoverCardTrigger asChild>
                        <button className="inline-flex items-center rounded-md bg-slate-100 text-slate-700 border border-slate-200 px-3 py-1.5 text-xs font-medium cursor-pointer hover:opacity-80 transition-opacity">
                          Pending
                        </button>
                      </HoverCardTrigger>
                      <HoverCardContent className="w-[90vw] max-w-[320px] bg-white border-slate-200 shadow-xl" align="start" sideOffset={8}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-slate-200 bg-slate-50/50">
                <th className="text-left py-4 px-6 font-semibold text-slate-900 text-sm">Case</th>
                <th className="text-left py-4 px-6 font-semibold text-slate-900 text-sm">Reported</th>
                <th className="text-left py-4 px-6 font-semibold text-slate-900 text-sm">Evidence</th>
                <th className="text-left py-4 px-6 font-semibold text-slate-900 text-sm">Reported Violations</th>
                <th className="text-left py-4 px-6 font-semibold text-slate-900 text-sm">Verdict</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="py-5 px-6">
                      <div className="min-w-0">
                        <div className="font-medium text-slate-900 truncate max-w-[220px] text-sm">
                          {senderName}
                        </div>
                    ...
                    <td className="py-5 px-6 text-slate-700 whitespace-nowrap text-sm">
