...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!imageUrl) {
      ...
      return NextResponse.json({ url: null, landingUrl: row?.landing_url || null, status: row?.landing_render_status || null, raw: row });
    }
    if (imageUrl.startsWith("http")) {
      console.log("/api/cases/[id]/landing-url:using_raw", { imageUrl: imageUrl.slice(0, 60) + "…" });
      return NextResponse.json({ url: imageUrl, landingUrl: row?.landing_url || null, status: row?.landing_render_status || null, mime: "image/png", raw: row });
    }
    ...
    return NextResponse.json({ url, landingUrl: row?.landing_url || null, status: row?.landing_render_status || null, mime: "image/png", raw: row });
  } catch (e) {
