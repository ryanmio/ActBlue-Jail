-- Add platform_status to get_homepage_stats
-- Includes platform status from platform_status table and unique sender count from last 30 days

-- First, create the platform_status table if it doesn't exist
CREATE TABLE IF NOT EXISTS platform_status (
  id TEXT PRIMARY KEY DEFAULT 'default',
  actblue_reporting_status TEXT NOT NULL DEFAULT 'active' CHECK (actblue_reporting_status IN ('active', 'offline')),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Insert default row if not exists
INSERT INTO platform_status (id) VALUES ('default') ON CONFLICT (id) DO NOTHING;

-- Update the get_homepage_stats function to include platform_status
