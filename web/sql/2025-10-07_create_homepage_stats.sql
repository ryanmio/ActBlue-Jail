-- Combined homepage stats: recent cases + worst offenders in one call
-- Returns JSON with both sections for optimal performance
...
-- Create index to speed up this query if it doesn't exist
create index if not exists idx_submissions_public_created 
  on submissions(public, created_at desc) 
  where public = true;
